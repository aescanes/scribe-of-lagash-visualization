...
**"Create" button next to the settings field** — there is no toolbar
...
the setting's **"Create"** button to write an empty table skeleton (one per
...
- **Settings** — `outlineFileName: ""` (empty = feature off) in
  ...
  field plus a "Create" button (`plugin.createOutlineFiles()`) in
...
- Whether `ensureOutlineFile` should also react to the file being deleted out
  ...
  "Create" in settings again; not watched).
